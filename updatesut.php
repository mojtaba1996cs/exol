...
include"cont.php";
$sql= mysqli_query($connect,"SELECT *  FROM `".$db_name."`.`students` where `Std_status`='1' and `Std_class`='الفصل الاول' ");
$result= mysqli_fetch_array($sql);
  if($result > 0){
  $Std_status="1";
  $Std_class="الفصل الثاني";
$result = mysqli_query($connect,"UPDATE `".$db_name."`.`students` SET  `Std_class` = '$Std_class' WHERE `students`.`Std_status` ='1' and `Std_class`='الفصل الاول' ");
if ($result){
...
  if (willDelete) {
  ...
  {
   echo'<script language="javascript">
  swal("لايمكن نقل كل الطلاب لعدم وجود طالب في الفصل الاول"
,{
icon:"info",
})
.then((willDelete) => {

...
include"cont.php";
$id=intval($_GET['id']);
 $Std_set=$_GET['std_set'];
$sql= mysqli_query($connect,"SELECT *  FROM `".$db_name."`.`students` where `Std_id`='$id' ");
$result= mysqli_fetch_array($sql);
  $Std_status="0";
  $id2=$id;
  $name=$result['Std_name'];
$result = mysqli_query($connect,"UPDATE `".$db_name."`.`students` SET  `Std_status` = 0 WHERE `students`.`Std_set` ='$Std_set' ");
if ($result){
	echo'<script language="javascript">
        swal("تم توقيف الطالب '.$name.'",{
    icon:"success",
})
.then((willDelete) => {
  if (willDelete) {
  ...
  echo'<script language="javascript">
 swal("فشل الايقاف .. اعاد المحاوله",{
  icon:"error",
})
.then((willDelete) => {
  if (willDelete) {
